...
String convertJsonToM3U(Map<String, dynamic> jsonData) {
  try {
    ...
    final generatedBy = meta['generated_by']?.toString() ?? "FukatMusic";
    ...
    // Custom FukatMusic metadata (ignored by standard M3U players).
    buffer.writeln("#FUKATMUSIC-GENERATED_BY: $generatedBy");
    buffer.writeln("#FUKATMUSIC-VERSION: $version");
    buffer.writeln("#FUKATMUSIC-EXPORTEDAT: $exportedAt");
    buffer.writeln("#FUKATMUSIC-NOTE: $note");
    buffer.writeln();
...
Map<String, dynamic> parseM3UToJson(String m3uContent) {
  try {
    ...
    for (final line in lines) {
      ...
      if (line.startsWith('#FUKATMUSIC-GENERATED_BY:')) {
        meta['generated_by'] = line.split(':').last.trim();
      } else if (line.startsWith('#FUKATMUSIC-VERSION:')) {
        meta['version'] = line.split(':').last.trim();
      } else if (line.startsWith('#FUKATMUSIC-EXPORTEDAT:')) {
        meta['exportedAt'] = line.split(':').last.trim();
      } else if (line.startsWith('#FUKATMUSIC-NOTE:')) {
