...
import 'package:fukatsongs/plugins/blocs/import/content_import_state.dart';
import 'package:fukatsongs/plugins/blocs/plugin/plugin_bloc.dart';
import 'package:fukatsongs/plugins/blocs/plugin/plugin_state.dart';
import 'package:fukatsongs/screens/widgets/snackbar.dart';
import 'package:fukatsongs/src/rust/api/plugin/models.dart';
import 'package:fukatsongs/screens/widgets/downloading_item.dart';
...
import 'package:fukatsongs/utils/download_types.dart';
...
class _ImportProcessScreenState extends State<ImportProcessScreen> {
  ...
  bool _downloadOnComplete = false;
            ...
            child: BlocConsumer<ContentImportCubit, ContentImportState>(
              listener: (context, state) {
                if (state.phase == ImportPhase.done) {
                  ...
                  if (_downloadOnComplete) {
                    final downloaderCubit = context.read<DownloaderCubit>();
                    for (final entry in state.tracks) {
                      if (entry.effectiveTrack != null) {
                        downloaderCubit.downloadSong(entry.effectiveTrack!, showSnackbar: false);
                      }
                    }
                  }
                }
    ...
    switch (state.phase) {
      case ImportPhase.idle:
        return _UrlInputView(
          ...
          downloadOnComplete: _downloadOnComplete,
          onDownloadOnCompleteChanged: (val) {
            setState(() {
              _downloadOnComplete = val;
            });
          },
        );
...
class _UrlInputView extends StatelessWidget {
  ...
  final bool downloadOnComplete;
  final ValueChanged<bool> onDownloadOnCompleteChanged;

  const _UrlInputView({
    super.key,
    required this.controller,
    required this.onSubmit,
    required this.downloadOnComplete,
    required this.onDownloadOnCompleteChanged,
  });
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<DownloaderCubit, DownloaderState>(
      builder: (context, downloaderState) {
        // Filter downloads to only those that belong to our imported tracks
        final importedTrackIds = state.tracks
            .where((t) => t.effectiveTrack != null)
            .map((t) => t.effectiveTrack!.id)
            .toSet();

        final relevantDownloads = downloaderState.downloads
            .where((d) => importedTrackIds.contains(d.task.mediaId))
            .toList();

        final total = relevantDownloads.length;
        final completed = relevantDownloads
            .where((d) => d.status.state == DownloadState.completed)
            .length;
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              if (state.collectionInfo != null)
                _CollectionHeader(info: state.collectionInfo!),
              const SizedBox(height: 24),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  value: progress,
                  minHeight: 8,
                  backgroundColor: Default_Theme.primaryColor1.withValues(alpha: 0.1),
                  valueColor: const AlwaysStoppedAnimation(Default_Theme.accentColor2),
                ),
              ),
              const SizedBox(height: 12),
              Text(
              ...
              Expanded(
                child: ListView.separated(
                  itemCount: relevantDownloads.length,
                  separatorBuilder: (_, __) => Divider(
                    color: Default_Theme.primaryColor1.withValues(alpha: 0.05),
                    height: 1,
                  ),
                  itemBuilder: (context, index) {
                    final download = relevantDownloads[index];
                    final track = state.tracks
                        .firstWhere((t) => t.effectiveTrack?.id == download.task.mediaId)
                        .effectiveTrack!;

                    return DownloadingCardWidget(
